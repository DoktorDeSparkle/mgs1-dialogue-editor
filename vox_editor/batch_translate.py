"""Full machine-translation pass over voxText-jpn.json into vox_editor_state.json.
...
from server import ROOT, STATE_PATH, load_config, load_json, state_lock, translate, write_state  # noqa: E402
...
def main():
    ...
    cands = {r["jpn_key"]: r for r in load_json(ROOT / "jpn_to_usa_candidates.json", [])}
    hung = {r["jpn_key"]: r for r in load_json(ROOT / "vox_matches_min2.json", [])}
    ...
    with state_lock():
        ...
        for key, entry in jpn.items():
            ...
            if key not in state["convs"]:
                conv = new_conv(entry)
                conv["usa_ref"] = ((cands.get(key) or {}).get("candidates") or [{}])[0].get("usa_key") or (hung.get(key) or {}).get("usa_key")
    ...
    total = len(jobs)
    print(f"{total} translations to run ({', '.join(engines)}), {args.workers} parallel per engine", flush=True)
    if not total:
        return
    ...
    flush()
    if errors:
        (ROOT / "batch_translate_errors.json").write_text(json.dumps(errors, ensure_ascii=False, indent=1))
        print(f"{len(errors)} failures written to batch_translate_errors.json — rerun to retry them", flush=True)
